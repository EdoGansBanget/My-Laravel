                    ...
                    <form action="{{ route('admin.profile') }}" method="GET">
                        ...
                        <div class="input-group">
                            <input type="search" name="search" class="form-control rounded"
                                placeholder="Cari profil lulusan" aria-label="Search" aria-describedby="search-addon" />
                            <button type="submit" class="btn btn-outline-primary"><i class="fas fa-search"></i></button>
                        </div>
                    </form>
                </div>
                <div class="col text-end">
                    <a class="btn btn-success" href="{{ route('admin.tambahProfile') }}"
            ...
            <table class="table mt-4">
                ...
                <tbody class="table-group-divider">
                    ...
                    <tr>
                        ...
                        <td>
                            <img style="width: 50px" src="{{ asset('/images/' . $profile->foto) }}" alt="foto profil">
                        </td>
                        <td>{{ $profile->nama }}</td>
                        <td>{{ $profile->prodi_lulusan }}</td>
                        <td>{{ $profile->gelar }}</td>
                        <td>{{ $profile->tahun_lulus }}</td>
                        <td>
                            <a class="btn btn-outline-warning" href="{{ route('admin.editProfile', $profile->id) }}"><i
                                    class="fas fa-pen"></i></a>
                            <a class="btn btn-outline-danger" href="{{ route('admin.deleteProfile', $profile->id) }}"><i
